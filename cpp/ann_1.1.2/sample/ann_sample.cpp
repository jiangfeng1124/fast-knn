...
bool readPt(istream &in, string &label, ANNpoint p)			// read point (false on EOF)
{
    in >> label;
	for (int i = 0; i < dim; i++) {
...
int main(int argc, char **argv)
{
    ...
    string label;
	while (nPts < maxPts && readPt(*dataIn, label, dataPts[nPts])) {
	...
	while (readPt(*queryIn, label, queryPt)) {			// read query points
...
void getArgs(int argc, char **argv)
{
	...
	while (i < argc) {							// read arguments
		if (!strcmp(argv[i], "-d")) {			// -d option
			...
			k = atoi(argv[++i]) + 1;			// get number of near neighbors
		}
